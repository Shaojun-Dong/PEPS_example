module DoubleLayerEnergy_type
	...
	use Tools
	implicit none 
	type(Tensor),allocatable::AuxUpOrLeft(:,:)
	type(Tensor),allocatable::AuxDownOrRight(:,:)
	integer::AuxFlag=0
...
contains
	...
	subroutine Phi_O_Phi_H(outA1,outA2,O,A1,A2)
		type(Tensor),intent(in)::O,A1,A2
		type(Tensor),intent(inout)::outA1,outA2
		type(Tensor)::C,U,S,V
	...
	subroutine Phi_H_Phi_two_site(outA,outB,A,B,R,L,phya,phyb,H)
		type(Tensor),intent(inout)::outA,outB
	...
	function ValueH(ith,jth,H,singleA,DoubleA,norm,phya,phyb)
		...
		type(Tensor),intent(in)::H,singleA(:,:),DoubleA(:,:)
		real*8,intent(in)::norm
		type(Tensor)::A1,A2
		integer::j,L1,L2,row,rowi
		type(Tensor),allocatable::Network(:,:)
	...
	function ContractEnergy(nodeA,H,Dc)
		...
		integer,intent(in)::Dc
		type(Tensor),allocatable::A(:,:),PEPS(:,:)
		integer::L1,L2
		L1=size(nodeA,1)
		L2=size(nodeA,2)
		allocate(A(L1,L2))
