module DoubleLayerEnergy_type
	...
	use Tools
	implicit none 
	type(fTensor),allocatable::AuxUpOrLeft(:,:)
	type(fTensor),allocatable::AuxDownOrRight(:,:)
	integer::AuxFlag=0
...
contains
	...
	subroutine Phi_O_Phi_H(outA1,outA2,O,A1,A2)
		type(fTensor),intent(in)::O,A1,A2
		type(fTensor),intent(inout)::outA1,outA2
		type(fTensor)::C,U,S,V
		character(len=100)::name1,name2
		integer::fermiarrow1,fermiarrow2
		name1=A1%getName('n')
		name2=A2%getName('n')
		...
		fermiarrow1=U%getFermiArrow(A1%getName('R'))
		fermiarrow2=A1%getFermiArrow(A1%getName('R'))
		if(fermiarrow1.ne.fermiarrow2)then
			call ReverseFermiArrow(U,A1%getName('R'),V,A2%getName('L'))
		end if
	...
	subroutine Phi_O_Phi_V(outA1,outA2,O,A1,A2)
		...
		fermiarrow1=U%getFermiArrow(A1%getName('D'))
		fermiarrow2=A1%getFermiArrow(A1%getName('D'))
		if(fermiarrow1.ne.fermiarrow2)then
			call ReverseFermiArrow(U,A1%getName('D'),V,A2%getName('U'))
		end if
	...
	subroutine Phi_H_Phi_two_site(outA,outB,A,B,R,L,phya,phyb,H)
		...
		fermiarrow1=U%getFermiArrow(name1)
		fermiarrow2=A%getFermiArrow(name1)
		if(fermiarrow1.ne.fermiarrow2)then
			call ReverseFermiArrow(U,name1,V,name2)
		end if
	...
	function ValueH(ith,jth,H,singleA,DoubleA,norm,phya,phyb)
		...
		type(fTensor),intent(in)::H,singleA(:,:),DoubleA(:,:)
		real*8,intent(in)::norm
		type(fTensor)::A1,A2
		integer::j,L1,L2,row,rowi
		type(fTensor),allocatable::Network(:,:)
	...
	function EnergyFunction(singleA,DoubleA,inH,Dc)
		...
		type(fTensor)::inH
		type(fTensor)::singleA(:,:)
		type(fTensor)::DoubleA(:,:)
		integer::Dc
	...
	function ContractEnergy(nodeA,H,Dc)
		...
		integer,intent(in)::Dc
		type(fTensor),allocatable::A(:,:),PEPS(:,:)
		integer::L1,L2
		L1=size(nodeA,1)
		L2=size(nodeA,2)
		allocate(A(L1,L2))
